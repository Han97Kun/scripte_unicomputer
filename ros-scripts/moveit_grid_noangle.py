...
timeout = 10.0
t = 4  #verweildauer der sonde am punkt
...
#TODO
    ...
    #edge clreanace rausnehmen
    # das liegt alles nich nicht mittig das raster 
...
def raster_scan(ctx, config, marker_pub, box_pose, box_size, probe_length=0.1):
    ...
    # Raster-Mitte (erste anzufahrende Position)
    x_center = x_start + width / 2.0
    y_center = y_start_centered + width / 2.0

    rospy.sleep(2)


          # =========================
            # 0) ZUERST: Mittel-Punkt (NICHT loggen)
            # =========================
    # --- 0) Erster Schritt: genau über dem Zylinderzentrum, 10 cm über Oberkante ---
    approach_above_m = float(config.get("approach_above_m", 0.10))  # 10 cm
    bx, by, bz = box_pose.pose.position.x, box_pose.pose.position.y, box_pose.pose.position.z
    z_circle_top = bz + (box_size[2] / 2.0)  # box_size[2] == height_strahler

    # Spitze exakt über dem Kreis (kein XY-Versatz), 10 cm über der Oberkante
    tip_over_cyl = np.array([bx, by, z_circle_top + approach_above_m])
    pose_over_cyl = compute_effector_pose_for_probe_tip(tip_over_cyl, base_quat, probe_length)
    ...
    if frac_over > 0.8:
        ...
        rospy.sleep(t)  # wenn du hier kein Verweilen willst: Zeile auskommentieren
        rospy.loginfo(f"[✓] Approach: über Kreiszentrum bei z={z_circle_top + approach_above_m:.3f} m")
    else:
        rospy.logwarn(f"[!] Approach über Kreis nicht planbar (fraction={frac_over:.2f}) – fahre direkt mit Raster fort.")
