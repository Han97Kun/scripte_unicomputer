...
timeout = 10.0
t = 3  #verweildauer der sonde am punkt
...
mkdir -p ~/repos
cd ~/repos
git clone git@github.com:Han97Kun/scripte_unicomputer.git
cd scripte_unicomputer

# 2) Git-Identität setzen (nur dieses Repo)
git config user.name "Han97Kun"
git config user.email "hkunze97@googlemail.com"

# 3) Deine Skripte aus catkin_ws reinkopieren (Beispielpfade anpassen!)
#   Beispiel: alle .py aus deinem Paket:
mkdir -p ros-scripts
rsync -av ~/catkin_ws/src/panda_moveit_config/scripts/*.py ./ros-scripts/

# 4) Commit & Push
git add -A
git commit -m "final rasterscan script und angepasste match cps to position"
git push
